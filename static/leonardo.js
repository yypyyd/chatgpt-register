...
const size = 20;
let leonardoCache = {};
const leonardoSelected = new Set();
let logTimer = null;
let logId = null;
...
async function load() {
  ...
  const r = await api('/api/leonardo/registrations?' + params);
  const d = await r.json();
  leonardoCache = {};
  (d.data || []).forEach(x => { leonardoCache[x.id] = x; });
  document.getElementById('rows').innerHTML = (d.data || []).map(rowHtml).join('')
    || '<tr><td colspan="7" style="text-align:center;color:var(--text-3)">暂无 Leonardo 数据</td></tr>';
...
function rowHtml(x) {
  ...
  return `
    <tr class="${leonardoSelected.has(x.id) ? 'row-sel' : ''}">
      <td class="col-check"><input type="checkbox" ${leonardoSelected.has(x.id) ? 'checked' : ''} onclick="toggleSelect(${x.id}, this.checked)"></td>
      <td>${esc(x.email)}</td>
      <td>${fmtTime(x.created_at)}</td>
      <td><span class="badge ${esc(x.status)}">${LEONARDO_STATUS[x.status] || esc(x.status)}</span></td>
      <td>${aliveCell(x)}</td>
      ...
      <td>
        ...
        <button class="icon-btn" title="导出 Cookie 字符串" ${canDownload ? '' : 'disabled'} onclick="downloadLeonardo(${x.id}, 'string')">
        ...
        <button class="icon-btn" title="导出 Cookie JSON" ${canDownload ? '' : 'disabled'} onclick="downloadLeonardo(${x.id}, 'json')" style="font-size:10px;font-weight:700">JSON</button>
...
async function loadProduce() {
  try {
    const r = await api('/api/leonardo/produce/status');
...
async function startProduce() {
  ...
  const r = await api('/api/leonardo/produce', {
    method: 'POST',
...
async function stopProduce() {
  if (!confirm('确定停止当前所有 Leonardo 生产任务?')) return;
  await api('/api/leonardo/produce/stop', { method: 'POST' });
  toast('已请求停止');
  loadProduce();
}
...
function toggleSelect(id, checked) {
  if (checked) leonardoSelected.add(id); else leonardoSelected.delete(id);
  syncBatchBar();
}
function toggleSelectAll(checked) {
  Object.keys(leonardoCache).forEach(id => {
    if (checked) leonardoSelected.add(Number(id)); else leonardoSelected.delete(Number(id));
  });
  load();
}
function clearSelection() { leonardoSelected.clear(); load(); }
function syncBatchBar() {
  const bar = document.getElementById('leonardo-batch');
  bar.style.display = leonardoSelected.size ? 'flex' : 'none';
  document.getElementById('leonardo-batch-count').textContent = '已选 ' + leonardoSelected.size + ' 项';
  const all = document.getElementById('leonardo-check-all');
  const ids = Object.keys(leonardoCache).map(Number);
  all.checked = ids.length > 0 && ids.every(id => leonardoSelected.has(id));
}

/* ===== 导出 Cookie（string 字符串 / json 对象 / array 批量数组；导出即出库） ===== */
async function downloadLeonardo(id, format) {
  await downloadByIds([id], format);
}
async function downloadSelected(format) {
  const ids = [...leonardoSelected];
...
async function downloadByIds(ids, format, unshippedOnly) {
  const r = await api('/api/leonardo/download', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const fallback = format === 'array' ? 'leonardo_cookies_array.json'
    : format === 'json' ? 'leonardo_cookies.json' : 'leonardo_cookies.txt';
...
async function delSelected() {
  const ids = [...leonardoSelected];
  if (!ids.length) return;
  if (!confirm('确定删除所选 ' + ids.length + ' 个 Leonardo 账号？')) return;
  for (const id of ids) {
    await api('/api/leonardo/registrations/' + id, { method: 'DELETE' });
    leonardoSelected.delete(id);
  }
  toast('已删除 ' + ids.length + ' 个');
  load();
}
async function del(id) {
  if (!confirm('确定删除 Leonardo 账号 #' + id + '？')) return;
  const r = await api('/api/leonardo/registrations/' + id, { method: 'DELETE' });
  if (!r.ok) return toast('删除失败', true);
  leonardoSelected.delete(id);
  toast('已删除');
  load();
}
async function deleteAllLeonardo() {
  if (!confirm('确定删除全部 Leonardo 账号记录？此操作不可恢复。')) return;
  if (!confirm('再次确认：将永久删除 Leonardo 注册中的全部记录。')) return;
  const r = await api('/api/leonardo/registrations', { method: 'DELETE' });
  const d = await r.json().catch(() => ({}));
  if (!r.ok) return toast('全部删除失败: ' + (d.error || r.status), true);
  leonardoSelected.clear();
  page = 1;
  toast('已删除 ' + (d.deleted || 0) + ' 个 Leonardo 账号');
  load();
  loadProduce();
}
...
async function refreshLog(silent) {
  ...
  const r = await api('/api/leonardo/registrations/' + logId + '/logs');
...
async function viewShot() {
  if (logId == null) return;
  const r = await api('/api/leonardo/registrations/' + logId + '/shot');
...
const LIVE_BASE = '/api/leonardo/registrations';
