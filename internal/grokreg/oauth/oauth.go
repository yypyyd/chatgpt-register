...
func (c *Client) Exchange(ctx context.Context, sso string) (Credential, error) {
	return c.ExchangeWithFlow(ctx, sso, nil)
}

// ExchangeWithFlow is Exchange with an optional observer invoked for each
// freshly-issued user_code. It lets the pipeline expose device-flow progress
// without leaking device_code or tokens.
func (c *Client) ExchangeWithFlow(ctx context.Context, sso string, onFlow func(DeviceFlow)) (Credential, error) {
	var last error
